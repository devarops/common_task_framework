...
| **Orchestrator** | A deterministic program that coordinates the flow: validates inputs, runs hidden tests as a PASS/FAIL gate, sends prompts to referees (only for passing solutions), aggregates referee scores, manages match outcomes, and communicates with the external ELO API. | No LLM; pure logic (Bash/Shell). |
